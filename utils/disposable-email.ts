...
export const isDisposableEmail = async (email: string) => {
  ...
  if (domains) {
    ...
    const domainPattern = new RegExp(domains.map(d => d.replace(/\./g, '\\.').replace(/\*/g, '.*')).join('|'), 'i');
    return domainPattern.test(emailDomain);
  }

  // Fall back to local JSON
  const localDomains = Object.keys(disposableEmailDomains as Record<string, boolean>);
  const localDomainPattern = new RegExp(localDomains.map(d => d.replace(/\./g, '\\.').replace(/\*/g, '.*')).join('|'), 'i');
